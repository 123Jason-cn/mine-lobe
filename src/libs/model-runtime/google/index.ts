...
export class LobeGoogleAI implements LobeRuntimeAI {
  ...
  constructor({ apiKey, baseURL, client, isVertexAi, id }: LobeGoogleAIParams = {}) {
    ...
    console.log('client ', client, { apiKey, httpOptions })
  ...
  async videoUpload(file: File) {
    const myfile: any = await this.client.files.upload({
      file: file,
  ...
  async chat(rawPayload: ChatStreamPayload, options?: ChatMethodOptions) {
    ...
    console.log('chat ', rawPayload)
    try {
      const payload = this.buildPayload(rawPayload);
      const { model, thinkingBudget } = payload;

      console.log('chat send text', rawPayload);
...
``
      ...
      console.log('geminiStreamResponse ', geminiStreamResponse)
  ...
  private convertContentToGooglePart = async (
    ...
    console.log('convertContentToGooglePart ', content)
  ...
  private convertOAIMessagesToGoogleMessage = async (
    ...
    const getParts = async () => {
      ...
      console.log('convertOAIMessagesToGoogleMessage ', content)
      const parts = await Promise.all(
        content.map(async (c) => await this.convertContentToGooglePart(c)),
      );
      console.log('parts ', parts)
      console.log('parts filter ', parts.filter(Boolean))
