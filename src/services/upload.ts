...
interface UploadFileToS3Options {
  ...
  onFileSizeTooLarge?: (fileSize: number) => void;
  pathname?: string;
...
class UploadService {
  ...
  uploadFileToS3 = async (
    file: File,
    { onProgress, directory, skipCheckFileType, onNotSupported, pathname, onFileSizeTooLarge }: UploadFileToS3Options,
    ...
    if (file.size > (MAX_FILE_SIZE * Math.pow(1024, 2))) {
      onFileSizeTooLarge?.(MAX_FILE_SIZE);
    ...
    if (file.type.startsWith('image')) {
      console.time('file.arrayBuffer');
      const fileArrayBuffer = await file.arrayBuffer();

      // 1. check file hash
      hash = sha256(fileArrayBuffer);
      console.timeEnd('file.arrayBuffer');
    } else {
      hash = file.name;
    }
    ...
    // 改为支持视频
    if (!skipCheckFileType && file.type.startsWith('video')) {
      await this.uploadToClientS3(hash, file, true);
      const data = await this.uploadVideoToClientS3(hash, file);
      return { data, success: true };
    }
  ...
  private uploadToClientS3 = async (hash: string, file: File, isVideo: boolean = false): Promise<FileMetadata> => {
    await clientS3Storage.putObject(hash, file, isVideo);
  ...
  private uploadVideoToClientS3 = async (hash: string, file: File): Promise<FileMetadata & { pathUrl: string }> => {
    console.log('file', file);

    console.time('append to formData');
    const formData = new FormData();
    formData.append('file', file);
    formData.append('fileType', file.type);
    formData.append('fileName', file.name);
    console.timeEnd('append to formData');

    console.log('formData ', formData)

    // 直接用 fetch 上传到 Next.js API
    ...
    try {
      ...
      console.error('upload video to google error', err);
    }
